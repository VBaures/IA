...
situation_terminale_ligne([]).
situation_terminale_ligne([L|Rest]):-
    ground(L),
    situation_terminale_ligne(Rest).

situation_terminale(_,[]).
situation_terminale(_Joueur, [L|Rest]) :-
    situation_terminale_ligne(L),
    situation_terminale(_Joueur,Rest). 
...
ligne_transpose(_,[],[]).
ligne_transpose(Index,[M1|MRest],[L1|LRest]):-
    nth1(Index,M1,L1),
    ligne_transpose(Index,MRest,LRest).

do_transpose(4,_,[]).
do_transpose(Index,M,[L1|LRest]):-
    ligne_transpose(Index,M,L1),
    NewIndex is Index+1,
    do_transpose(NewIndex,M,LRest).

colonne(C,M) :- 
    do_transpose(1,M,NewM),
    ligne(C,NewM).
